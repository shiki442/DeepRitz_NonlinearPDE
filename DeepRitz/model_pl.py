...
class DeepRitzSystem(pl.LightningModule):
    def __init__(self, cfg):
        ...
        act = cfg.net.act if hasattr(cfg.net, 'act') else 'ReLU6p'

        self.solution_net = SolutionNet(in_features=dim, out_features=1, block_width=block_width, n_blocks=n_blocks, act=act)
    ...
    def configure_optimizers(self):
        optimizer = optim.Adam(self.parameters(), lr=self.cfg.training.lr, betas=self.cfg.training.betas, eps=1e-08)
        scheduler = optim.lr_scheduler.ReduceLROnPlateau(
            optimizer, patience=self.cfg.training.patience, factor=self.cfg.training.gamma, min_lr=1e-6
        )
        return {
            "optimizer": optimizer,
            "lr_scheduler": {
