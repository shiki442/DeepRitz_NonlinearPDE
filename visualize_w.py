"""
Eq5 可视化脚本：不同网络宽度实验的结果分析
完成以下任务：
1. 读取 tensorboard 日志提取相关数据
2. 绘制 4 个不同网络宽度下的 error-epoch 曲线（同一个折线图，光滑化）
3. 绘制四个网络宽度下的解的热力图以及误差的热力图（subfigure，统一 colorbar）
4. 绘制网络宽度 - 误差关系折线图（基于 checkpoint 数据）
"""
...
# ============================================================
EXPERIMENTS = {
    10: {'version': 'version_7', 'log_dir': './logs/eq5/lightning_logs/version_7', 'ckpt_dir': './checkpoints/eq5_w10'},
    20: {'version': 'version_8', 'log_dir': './logs/eq5/lightning_logs/version_8', 'ckpt_dir': './checkpoints/eq5_w20'},
    40: {'version': 'version_9', 'log_dir': './logs/eq5/lightning_logs/version_9', 'ckpt_dir': './checkpoints/eq5_w40'},
    80: {'version': 'version_0', 'log_dir': './logs/eq5/lightning_logs/version_0', 'ckpt_dir': './checkpoints/eq5_w80'},
}
...
# ============================================================
# 任务 1: 读取 TensorBoard 日志并绘制 error-epoch 曲线（光滑化）
# ============================================================
def plot_error_epoch_curves():
    """绘制 4 个不同网络宽度下的 error-epoch 曲线（光滑化）"""
    print("\n" + "="*60)
    print("任务 1: 读取 TensorBoard 日志并绘制 error-epoch 曲线")
    print("="*60)
    ...
    for width, color in zip(WIDTHS, COLORS):
        ...
        try:
            ...
            # 光滑化处理
            values_smooth = uniform_filter1d(values, size=10, mode='nearest')

            # 保存数据
            all_data[width] = {'epochs': epochs, 'errors': values, 'errors_smooth': values_smooth}

            # 绘制光滑后的曲线
            ax.plot(epochs, values_smooth, label=f'w={width}', linewidth=1.5, color=color, linestyle='-', alpha=0.7)
    ...
    ax.legend(title="Network Width", fontsize=10)
    ax.grid(True, linestyle='--', alpha=0.3, color='gray')
    ax.set_yscale('log')

    plt.tight_layout()
    save_path = os.path.join(OUTPUT_DIR, 'eq5_w_error_epoch_curves.png')
...
def plot_heatmaps():
    """绘制四个网络宽度下的解的热力图以及误差的热力图"""
    print("\n" + "="*60)
    print("任务 2: 绘制解和误差热力图")
    print("="*60)
    ...
    for idx, width in enumerate(WIDTHS):
        ...
        try:
            ...
            best_errors[width] = best_error
        except:
            best_errors[width] = None
    ...
    # 绘制热力图
    for idx, width in enumerate(WIDTHS):
        if width not in u_nn_all:
            continue

        u_nn = u_nn_all[width]
        error = error_all[width]
        best_error = best_errors.get(width)

        # 第一行：数值解
        im0 = axes[0, idx].pcolormesh(X, Y, u_nn, cmap='coolwarm', shading='auto',
                                       vmin=vmin_u, vmax=vmax_u)
        axes[0, idx].set_title(f'w={width}', fontsize=10)
    ...
    save_path = os.path.join(OUTPUT_DIR, 'eq5_w_heatmaps_comparison.png')
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    print(f"\n已保存：{save_path}")
    plt.close(fig)
...
def plot_width_vs_error(best_errors):
    """绘制网络宽度 - 误差关系折线图（基于 checkpoint 数据）"""
    print("\n" + "="*60)
    print("任务 3: 绘制网络宽度 - 误差关系图")
    print("="*60)
    ...
    for width in WIDTHS:
        if width in best_errors and best_errors[width] is not None:
            widths.append(width)
            errors.append(best_errors[width])

    print("\nCheckpoint 最佳误差:")
    for width, err in zip(widths, errors):
        print(f"  w={width}: val_error={err:.6e}")

    # 绘制网络宽度 - 误差关系图
    ax.plot(widths, errors, 'o-', linestyle='--', linewidth=2, markersize=8, color='#f59e0b', alpha=0.7)

    # 标注数据点
    for width, err in zip(widths, errors):
        ax.annotate(f'{err:.2e}', xy=(width, err),
                     xytext=(5, 5), textcoords='offset points', fontsize=9)

    ax.set_xlabel("Network Width (w)", fontsize=12)
    ax.set_ylabel("Validation Error", fontsize=12)
    ax.set_title("Validation Error vs Network Width", fontsize=14)
    ax.grid(True, linestyle='--', alpha=0.3, color='gray', which='minor')
    ax.set_xscale('log')
    ax.set_yscale('log')

    save_path = os.path.join(OUTPUT_DIR, 'eq5_w_width_vs_error.png')
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    print(f"\n已保存：{save_path}")
    plt.close(fig)
...
def main():
    ...
    # 任务 3: 绘制网络宽度 - 误差关系图
    plot_width_vs_error(best_errors)
